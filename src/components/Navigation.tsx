...
const Navigation = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setShowNavigation(
        scrollPosition > heroHeight - 100 && scrollPosition > 500
      );
  ...
  return (
    <nav
      className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 bg-background/90 backdrop-blur-md border border-border rounded-full px-6 py-3 shadow-lg transition-transform duration-300 ${
        showNavigation ? "translate-y-0" : "-translate-y-20"
      }`}
    >
